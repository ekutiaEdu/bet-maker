...
from app.routers.dto.bet import CreateBet, BetCreatedResult
from app.services.bet_service import BetService

bet_router = APIRouter(prefix="/bets", tags=["bets"])


@bet_router.get("/", response_model=list[Bet], status_code=HTTP_200_OK, name="bets:get_all")
async def get_all_bets(bet_service: BetService = Depends(get_bet_service)) -> Any:
    return await bet_service.get_all_bets()


@bet_router.post("/", response_model=BetCreatedResult, status_code=HTTP_201_CREATED, name="bets:add")
async def add_bet(bet: CreateBet, bet_service: BetService = Depends(get_bet_service)) -> BetCreatedResult:
